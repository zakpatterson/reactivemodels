...
object Iteratees {
  ...
  def withLimitIdentity[FROM](limit: Int): Iteratee[FROM, List[FROM]] =
    withLimit[FROM, FROM](limit)(x => x)

  def collectIteratee[A](implicit ec : ExecutionContext) : Iteratee[A, List[A]] =
    Iteratee.fold(List[A]())((acc, a) => a :: acc)
}
